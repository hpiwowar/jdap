...
tmp$variable <- letters[gsub("Q_", "", tmp$variable) %>% gsub("5_", "", .) %>% as.integer]
p <- plot_likert(tmp, response='value', var_x='PolicyAdoptedAtPublication', var_facet="variable", response_order=responses, above_mid="Recommends online public archiving", na.label="No response", xlab="% Responses", ylab="JDAP Policy", include.N=FALSE)
#p <- p + theme(axis.text.x=element_text(angle=-90, size=8))
...
x <- group_by(cur_d, value, PolicyAdoptedAtPublication, label_PolicyAdoptedAtPublication) %>% dplyr::summarize(N=length(value))
p <- ggplot(x, aes(label_PolicyAdoptedAtPublication, N, group=value, fill=value))+geom_bar(stat="identity", position="dodge")+ylab("Number of Responses")+ylab("JDAP Policy")
...
levels <- c("Journal supplementary information", "A repository for data or research output", "A personal or lab website",
            "The entire dataset is in the body of the paper", "Not applicable", "Other (please specify)")
cur_d$value <- ordered(cur_d$value, levels=levels)
...
tmp <- melt(d, measure.vars=paste0('Q_5_',c(1:3) ))
tmp$variable <- gsub("Q_", "Question ", tmp$variable)
...
cnames <- paste0('Q_6_',1:6)
x <- d[,cnames]
new_cnames <- apply(x, 2, function(x) unique(x[x!=""])) 
colnames(x) <- new_cnames
x <- x!=""
x
...
```{r, eval=FALSE, fig.width=7, fig.height=5, echo=FALSE}
